...
export const Detail = ({convertPrice, cart, setCart}) => {
  ...
  useEffect(() => {
    getProducts().then((data) => {
     setProduct(data.data.product.find((product) => product.id === parseInt(id))
     ); 
    });
  }, [id])
 //장바구니에 물건넣기 (같은상품 중복물건)
...
//장바구니에 물건넣기
  ...
  const found = cart.filter((el) => el.id === cartItem.id);
            ...
            <div className={styles.sum}>
              ...
              <div className={styles.total_info}>
                <span className={styles.total}>
                  총 수량 <span className={styles.total_count}>{count}1개</span>
                </span>
